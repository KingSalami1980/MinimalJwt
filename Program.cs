...
app.MapPost("/login",
    ...
    .Accepts<UserLogin>(MimeTypes.Application.Json)
    .Produces<string>();

app.MapPost("/create",
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Administrator")]
    (Movie movie, IMovieService service) => Create(movie, service))
    .Accepts<Movie>(MimeTypes.Application.Json)
    .Produces<Movie>(statusCode: 200, contentType: MimeTypes.Application.Json);
...
app.MapGet("/list",
    ...
    .Produces<List<Movie>>(statusCode: 200, contentType: MimeTypes.Application.Json);
...
IResult Login(UserLogin user, IUserService service)
{
        ...
        !string.IsNullOrEmpty(user.Password))
    {
        ...
        if (loggedInUser is null) return Results.NotFound("Use not found!"); // Used only as an example. Normally would use EF Core to validate against DB.
